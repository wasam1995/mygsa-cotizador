...
import type { ParametrosFiscales, PlantillaCotizacion } from '@/lib/types';
...
export default async function PlantillasPage() {
  ...
  const [{ data }, { data: parametros }] = await Promise.all([
    supabase.from('plantillas_cotizacion').select('*').order('nombre'),
  ...
  return (
    <div>
      ...
      <PlantillasClient plantillas={(data ?? []) as PlantillaCotizacion[]} parametros={parametros as ParametrosFiscales} />
    </div>
  );
}
